import Player, { canShootBullet, PlayerInput, shootBullet } from "../shared/player";
...
import ParticleSet, { fromPlayer, PARTICLE_TIME } from "@shared/particles";
import fade from "@display/fade";
import { authenticateDiscord } from "./discord";
...
export default interface Client {
	...
	discordAuth: Awaited<ReturnType<DiscordSDK["commands"]["authenticate"]>> | null;
...
export function initClient(canvasId: string, authCode: string): Client {
...
export async function setup(client: Client) {
	...
	client.discordAuth = await authenticateDiscord(target, client.discordAuthCode);
	client.discordAuthCode = "";

	client.connection = createConnection(target, client.performance.now(), client.discordAuth.user.username);
