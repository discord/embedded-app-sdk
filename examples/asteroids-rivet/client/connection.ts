import { ClientSideSocket, EndLife, Init, StopUpdates, Update } from "../shared/socket";
...
export function createConnection(connectionTarget: ConnectionTarget, now: number, username?: string): Connection {
	return {
		socket: getSocketForConnectionTarget(connectionTarget, username),
...
function getSocketForConnectionTarget(target: ConnectionTarget, username?: string): ClientSideSocket {
	const secure = target.port.isTls;
	const token = target.player.token;

	if (!target.port.host) throw new ConnectionError("Matchmaker did not provide a host to connect to.");

	const splitHost = target.port.hostname.split(".");
	const host = `${location.host}:${target.port.port}`;
	const socketIoPath = `/ws/${splitHost[2]}/${splitHost[0]}/socket.io`;
...
export function setupListeners(connection: Connection, client: Client, performance: Performance) {
	...
	connection.socket.once(
		"init",
		(data: Init, callback) => handleInit(client, connection, data, callback, performance.now()),
	);
...
function handleInit(client: Client, connection: Connection, data: Init, callback: (id: string) => void, now: number) {
	console.log(`Received init message @ ${client.performance.now()}`);
